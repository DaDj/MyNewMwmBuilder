...
namespace MwmBuilder
{
  public class MyModelBuilder
  {
      ...
      IMyBuildLogger logger)
    {
      ...
      {
        string path = Path.Combine(directoryName, withoutExtension + MyModelBuilder.LOD_POSTFIX + (object) (index + 1)) + ".fbx";
        string str2 = Path.Combine(path1, withoutExtension + MyModelBuilder.LOD_POSTFIX + (object) (index + 1));
        if (File.Exists(path))
        {
      ...
      {
        string path2 = withoutExtension + MyModelBuilder.LOD_POSTFIX + (object) 0;
