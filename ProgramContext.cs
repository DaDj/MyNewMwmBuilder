...
namespace MwmBuilder
{
  public class ProgramContext
  {
    ...
    public int Work(object data, IMyBuildLogger[] loggers = null)
    {
      ...
      this.m_args.RegisterArg("gss", "SHAREDSTUB", true.ToString(), "Generate shared stub and save LOD0 with postfix _LOD0");
    ...
    private void ProcessFileSafe(string file)
    {
      try
      {
        ...
        if (this.m_args.GetValue("do") != null)
          overrideLods = true;
        bool result = true;
        bool.TryParse(this.m_args.GetValue("gss"), out result);
        if (this.ProcessFile(file, ProgramContext.OutputDir = this.m_args.GetValue("o"), ProgramContext.m_vars, this.m_args.GetValue("e") != null, this.m_args.GetValue("f") != null, this.m_args.GetValue("checkOpenBoundaries") != null, overrideLods ? this.m_args.GetValue("do") : this.m_args.GetValue("d"), overrideLods, result))
        {
      ...
      bool genSharedStub)
    {
      ...
      this.ProcessItem(itemInfo, outputDir, havokCollisionShapes, checkOpenBoundaries, lodDistances1, overrideLods, genSharedStub);
      if (exportXml)
      {
      ...
      bool genSharedStub)
    {
      if (item.Configuration == null)
        ...
        new MyModelBuilder().Build(item.Path, "tmp", outputDir, item.Configuration, havokCollisionShapes, checkOpenBoundaries, lodDistances, overrideLods, genSharedStub, new Func<string, MyMaterialConfiguration>(ProgramContext.GetMaterialByRef), (IMyBuildLogger) ProgramContext.m_logger);
    }
